...
Non-image binary responses and binary images that do not fit inline are written
without parsing to a generated file in an OS temporary directory, using
owner-only directory and file modes where the platform supports them. The
returned path is untrusted content: callers must not open or execute it
automatically. These files are removed after one hour or when the extension
process exits, whichever happens first; failed timed cleanup is retried while
